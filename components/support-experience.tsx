...
import {
  ...
  PackageCheck, RefreshCcw, ShieldCheck, Sparkles, Truck, UserRound, Wrench,
} from 'lucide-react';
...
function RecommendationCard({ card }: { card: Extract<ContentCard, { kind: 'recommendations' }> }) {
  return <div className="mt-3 space-y-2.5" aria-label="Personalized book recommendations">{card.recommendations.map((book, index) => <article key={book.id} className="result-card overflow-hidden p-4"><div className="flex gap-3.5"><BookPlaceholder /><div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-2"><div><p className="text-[10px] font-bold uppercase tracking-[0.12em] text-primary">Match {index + 1}</p><h3 className="mt-1 font-heading text-lg font-semibold leading-5">{book.title}</h3><p className="mt-1 text-xs text-muted-foreground">{book.author}</p></div><span className="text-sm font-semibold">{currency(book.price)}</span></div><div className="mt-2 flex flex-wrap gap-1.5">{book.genres.slice(0, 3).map((genre) => <Badge key={genre} variant="secondary" className="rounded-full text-[10px]">{genre}</Badge>)}<Badge className="rounded-full bg-emerald-50 text-[10px] text-emerald-700 hover:bg-emerald-50">In stock</Badge></div><p className="mt-3 text-xs leading-5 text-[#514a42]"><strong>Why it fits:</strong> {book.reasoning || book.pitch || book.description}</p></div></div></article>)}</div>;
}
...
function ResultCard({ card, onAction }: { card: ContentCard; onAction: (message: string) => void }) {
  ...
  if (card.kind === 'recommendations') return <RecommendationCard card={card} />;
